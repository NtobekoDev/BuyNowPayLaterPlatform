...
namespace BNPLService.Controllers
{
    ...
    public class BNPLController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task< IActionResult >CreateApplication([FromBody] BNPLAppCreateDto dto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
            var application = await _bnplManager.CreateApplication(dto.UserId, dto.ProductId);
